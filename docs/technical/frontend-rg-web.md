...
- **Hero background video (PR4, marketing verdict — video only in Hero; DiscordCTA later):** a decorative `<video>` overlays the `<picture>` still inside `#hero-art` (both are children, so the GSAP scroll-mask scale 1.3→1.0 applies to the moving art). Pattern: `autoplay muted loop playsInline preload="none" poster="/hero/cityscape-16x9-2560.avif"` — the **AVIF poster IS the LCP** (preloaded via the two `<link>`s), the video loads after and never blocks LCP. Sources per breakpoint: `<source media="(min-width: 768px)" src="/hero/hero-16x9-loop.mp4">` (1280×720, 21.9s seamless loop) / `<source media="(max-width: 767px)" src="/hero/hero-9x16.mp4">` (720×1280, object-cover fills). `aria-hidden="true"` + `tabIndex={-1}` (decorative, never focused, `#hero-art` is pointer-events none). **Reduced-motion:** CSS-only kill — under `prefers-reduced-motion: reduce` the `.hero-video` layer is `display: none`, so the still `<picture>` shows (no autoplay attempt, no JS check). Weight budgets: ≤4MB desktop / ≤2MB mobile (hero-16x9-loop.mp4 = 3.05MB, hero-9x16.mp4 = 1.03MB — H.264, no audio track). Encode is a one-off ffmpeg step — NOT a runtime dependency. **Seamless loop technique (crossfade circular):** the source master has a start AND an end (not a native loop). To connect the last frame back to the first with constant forward motion (no cut, no boomerang reverse), ffmpeg `xfade` fades the final `D=1.2s` into the first `D=1.2s` of the same clip: `[0:v]split[a][b]; [a]trim=duration=1.2,setpts=PTS-STARTPTS[head]; [b]trim=duration=<TOTAL>,setpts=PTS-STARTPTS[main]; [main][head]xfade=transition=fade:duration=1.2:offset=<TOTAL-1.2>[v]` → output duration ≈ TOTAL, weight ≤4MB at CRF 27. The loop point becomes a dissolve between the tail and the head, so the browser `loop` attribute restarts into the opening frame imperceptibly. Generation: **Veo 3.1 image-to-video** from the cityscape (8s/1080p, start frame exported PNG/WebP — Veo input cannot be AVIF; 21.9s master is a user Veo output). Full record: [`hero-cityscape-prompt.md`](hero-cityscape-prompt.md) §Video record.
