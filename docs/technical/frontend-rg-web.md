...
| 2 — GSAP/Lenis scrub | `HeroScrollFx` (client) + ADR 0009 deps | Hero scroll choreography — **overfill REMOVED (hero at natural size, scale 1.0 — user preference)**: P1 mask/art tween is an identity, the choreography reduces to P2 wash + headline → P3 content parallax-out + cue | PR2; bails out entirely under `prefers-reduced-motion: reduce` (static final state, native scroll, no Lenis) |
...
- `HeroScrollFx.tsx` is the ONLY consumer of `gsap`/`lenis`. Pattern: `gsap.context(..., "#hero")` + revert, Lenis with `anchors: true`, `prefers-reduced-motion` bailout. **Trigger is `self.scope`** (the scoped element itself — NOT the selector `"#hero"`: inside a context that selector resolves as `#hero inside #hero` → "Element not found" warning). Scrubs a normalized 1.0 timeline with **overfill = 1.0** (user preference — hero at natural size; the 1080p video shows crisp at full resolution): P1 mask/art tween is an identity (`scale 1 → 1`, `--bg-zoom` 100% → 100% inert so the text-clip mask stays aligned), then P2 wash + headline (40–70%) → P3 content parallax-out + cue fade (0.15–0.30 / 70–100%).
...
- **Hero background video (PR4, marketing verdict — video only in Hero; DiscordCTA later; FINAL state 2026-09-08):** a decorative `<video>` overlays the `<picture>` still inside `#hero-art` (both are children, so the GSAP scroll-mask scale applies to the moving art). Pattern: `autoplay muted loop playsInline preload="none" poster="/hero/hero-16x9-poster.avif"` — the **poster is frame 0 of the FINAL desktop video** (34.6 KB, MUST match the video's first frame so there's no visual jump at start; regenerate after every re-encode). The **LCP stays the preloaded `<picture>` AVIF ladder** (`cityscape-16x9-2560.avif` via the two media-scoped `<link>`s); the video loads after and never blocks LCP. Sources per breakpoint: `<source media="(min-width: 768px)" src="/hero/hero-16x9-1080.mp4">` (1920×1080, 17.04s, 24fps, H.264 CRF 31, no audio, 2.91 MB — ≤4MB ✓) / `<source media="(max-width: 767px)" src="/hero/hero-9x16-loop.mp4">` (720×1280, 8s, H.264 CRF 26, no audio, 1.49 MB — ≤2MB ✓, object-cover fills). `aria-hidden="true"` + `tabIndex={-1}` (decorative, never focused, `#hero-art` is pointer-events none). **Reduced-motion:** CSS-only kill — under `prefers-reduced-motion: reduce` the `.hero-video` layer is `display: none`, so the still `<picture>` shows (no autoplay attempt, no JS check). Encode is a one-off ffmpeg step — NOT a runtime dependency (full WHY/HOW: §Hero video pipeline below). **Desktop loop technique (FINAL):** the 21.9s 720p Veo master was upscaled locally to 1080p (**Real-ESRGAN ncnn-vulkan** x4 → lanczos downscale to 1920×1080, 525 frames, ~17s/frame on GTX 1650S), then **trimmed to the window 1.0→19.0s** (removes the helicopter take-off at 0–1s and the end fade at 19–21s — the first attempt with the full master produced a visible "rebote" where the helicopters took off twice), then **crossfade circular** D=1.2s, offset≈17.8 (ffmpeg `xfade` — the tail dissolves into the head, so the browser `loop` restarts into the opening frame imperceptibly with constant forward motion) → **17.04s** seamless-ish loop at CRF 31. **Mobile loop technique (FINAL — DIFFERENT from desktop):** the 8s mobile clip is NOT a native loop; the circular crossfade did NOT work there (visible seam — the clip's start and end don't connect). FINAL approach: keep the full 8s original + `fade=t=in:st=0:d=0.4,fade=t=out:st=7.6:d=0.4` (**dip to black**) — the black masks the loop restart. **Pattern: if the clip isn't a native loop and crossfade seams show, use dip-to-black fades instead.** Poster = frame 0 of the FINAL file (regenerate after every re-encode). Generation: **Veo 3.1 image-to-video** from the cityscape (8s/1080p, start frame exported PNG/WebP — Veo input cannot be AVIF; the 21.9s master is a user Veo output). Full record: [`hero-cityscape-prompt.md`](hero-cityscape-prompt.md) §Video record.
...
| `public/hero/hero-16x9-1080.mp4` (1920×1080, 17.04s, 24fps, H.264 CRF 31, no audio, 2.91MB) + `public/hero/hero-9x16-loop.mp4` (720×1280, 8s, H.264 CRF 26, no audio, 1.49MB) + `public/hero/hero-16x9-poster.avif` (frame 0, 34.6 KB) | **FINAL 2026-09-08 (Veo 3.1 → Real-ESRGAN upscale → ffmpeg)** (integrated — video hero, see Track 2) | **Veo 3.1** (`veo-3.1-generate-preview`, Quality 1080p) image-to-video from the cityscape (16:9 + 9:16 separate clips), 8s each (Veo 3.1 image-to-video only supports 8s; separate generations, skyline may differ — deviation), no audio, 24fps; start frame exported PNG/WebP (Veo input cannot be AVIF). **Upscale (desktop):** Real-ESRGAN ncnn-vulkan x4 → lanczos downscale to 1920×1080 (525 frames, ~17s/frame GTX 1650S). **Desktop loop:** trim window 1.0→19.0s + crossfade circular D=1.2 offset≈17.8 → 17.04s, CRF 31. **Mobile:** full 8s + dip-to-black fade in/out 0.4s (NOT crossfade — visible seam), CRF 26. **Poster = frame 0 of the FINAL desktop file** (regenerate after every re-encode). One-off ffmpeg/Real-ESRGAN — NOT runtime deps. Budgets ≤4MB desktop / ≤2MB mobile (2.91MB / 1.49MB ✓). Full record: [`hero-cityscape-prompt.md`](hero-cityscape-prompt.md) §Video record. |
...
Encode with sharp (current contract — PR4, supersedes the PR2 q70/q66 record): clean **JPEG→AVIF** from the regenerated `.src-assets/hero/{pc,movil}.jpeg`, AVIF `effort: 9`, q82 (16:9 all rungs + 9:16 640/1280) / q78 (9:16 1920), WebP q80 effort 6 at EVERY served width, 16:9 ladder 640/1280/1920/2400/2560w + 9:16 640/1280/1920w, `sizes="130vw"` on 16:9 (9:16 `100vw` — deviation G). Raw JPEGs must never be served or committed (masters live in the gitignored `.src-assets/hero/`). Video: ffmpeg H.264 no-audio, CRF 31 (desktop 17.04s loop) / CRF 26 (mobile 8s dip-to-black), plus the Real-ESRGAN upscale step — all one-off, not runtime deps (see §Hero video pipeline for WHY/HOW).

## Hero video pipeline (Real-ESRGAN + ffmpeg + Katana) — WHY and HOW

Project requirement (Engram obs #1030): every technology used must be documented with its reason (WHY) and its workings (HOW) — the public repo is a traceable engineering showcase, not just code. This section is the record for the hero video tooling. All of it is **ONE-OFF build tooling** (temp scripts under `Temp\opencode`, never committed) — NOT a runtime dependency of rg-web (zero-dep policy, ADR 0002).

### Real-ESRGAN ncnn-vulkan (local GPU upscale, 720p → 1080p)

**WHY:** the Veo master is 720p — soft at full-bleed 1920×1080 next to the crisp 2560w AVIF still. The user wanted the video to match the art sharpness. Real-ESRGAN ncnn-vulkan is free, runs locally on the GPU (assets never leave the machine — pre-launch confidentiality), and needs no account or API key.

**HOW:** one-off pipeline (temp scripts `upscale-v5.ps1` / `runner-v4.ps1`):
1. Extract the 21.9s master to **525 frames** (ffmpeg, 24fps).
2. `realesrgan-ncnn-vulkan -i <frame> -o <out> -n realesrgan-x4plus -s 4` — x4 upscale to 2880p, then ffmpeg **lanczos** downscale to 1920×1080. **Pitfall:** x4 THEN downscale — a direct x2 with x4plus produces blocky artifacts ("cuadros").
3. ~17s/frame on a GTX 1650S → ~2.5h for 525 frames. Run in the FOREGROUND (`& $exe ...` directly) — **never `Start-Process`** (hangs silently; background pipelines fail systematically).
4. Reassemble to `video/hero-1080-raw.mp4` (ffmpeg, no audio).

Known limitation: per-frame upscaler has no temporal memory → shimmer/flicker risk on water, neons and night skies; the hero's slow atmospheric drift masks it (verified acceptable). Alternatives + cost arithmetic (incl. the "generate native 1080p with Veo instead of upscaling" insight): `radargta-business/marketing/herramientas/upscaler-online.md` (private).

### ffmpeg (the only video tooling)

**WHY:** one tool covers the whole pipeline — extraction, re-assembly after upscale, trim, crossfade, fades, encode. No video runtime dep is introduced: browsers play the H.264 output natively via `<video>`.

**HOW:** commands used (one-off, temp scripts):
- Extraction: `ffmpeg -i loop-master.mp4 -vf fps=24 frames-src/f_%04d.png`
- Re-assembly after upscale: `ffmpeg -framerate 24 -i frames-upscaled/f_%04d.png -c:v libx264 -crf 31 -pix_fmt yuv420p -movflags +faststart -an video/hero-1080-raw.mp4`
- Desktop trim + crossfade circular (window 1.0→19.0s of the 21.9s master, then dissolve tail into head): `[0:v]trim=start=1:end=19,setpts=PTS-STARTPTS,split[a][b]; [a]trim=duration=1.2,setpts=PTS-STARTPTS[head]; [b]trim=duration=<TOTAL>,setpts=PTS-STARTPTS[main]; [main][head]xfade=transition=fade:duration=1.2:offset=<TOTAL-1.2>[v]` — main first, head second (the first attempt with `[end][begin]` order produced a ~1.25s clip, wrong). Output ≈ 17.04s, H.264 CRF 31, no audio, 24fps, yuv420p + faststart.
- Mobile dip-to-black (clip NOT a native loop — the crossfade seam showed; the black masks the loop restart): `-vf "fade=t=in:st=0:d=0.4,fade=t=out:st=7.6:d=0.4"`, H.264 CRF 26, no audio.

### Katana Video (free.upscaler.video) — future shortcut for Shorts

**WHY:** browser-based WebGPU upscaler — local, private, no account, open source. Evaluated as the fast path for weekly short-form content (the local x4 pipeline costs ~2.5h of GPU on this machine per video). Complement, not replacement: the hero stays on the local Real-ESRGAN pipeline; Katana targets Shorts.

**HOW:** evaluation record in `radargta-business/marketing/herramientas/katana-upscaler.md` (private, Spanish). Apply to Shorts first; revisit for the hero only if the per-frame shimmer ever shows on a faster clip.
...
| 24 | Hero background video present: `<video>` element + frame-0 AVIF poster + per-breakpoint sources | `rg "hero-video\|poster=\"\/hero\/hero-16x9-poster.avif\"" components/Hero.tsx app/globals.css` + `rg "hero-16x9-1080.mp4\|hero-9x16-loop.mp4" components/Hero.tsx` (sources with `media` per breakpoint) | PR4 |
...
| 28 | Countdown mobile overflow fix present: vw-driven clamps + `min-width: 0` inside a `@media (max-width: 767px)` block placed AFTER the base count rules (so it wins the cascade) | `rg "clamp\(1.6rem, 8.5vw, 3.2rem\)\|clamp\(1.4rem, 6.5vw, 2.6rem\)\|min-width: 0" app/globals.css` (each must sit inside the `@media (max-width: 767px)` block) | PR4 (fix) |
| 29 | Hero at natural size: overfill removed (identity scale 1 / `--bg-zoom` 100% inert) + `self.scope` trigger (kills the "Element not found" warning) | `rg "trigger: self.scope" components/HeroScrollFx.tsx` + `rg "scale: 1, duration: 0.4" components/HeroScrollFx.tsx` (identity tween, no 1.3) | PR4 (fix) |
...
- [ ] Scrub choreography: hero at **natural size (overfill 1.0)** at top — no zoom, mask/art tween is an identity; wash fades 40–70%, content parallax-out 70–100%, cue fades 0.15–0.30 (PR2 + final overfill fix; verify in browser; DevTools console must show NO "Element not found" warning — `self.scope` trigger).
...
- [ ] Hero background video (FINAL): **desktop 16:9** plays the muted loop (`hero-16x9-1080.mp4`, 1920×1080) with the frame-0 poster (`hero-16x9-poster.avif`) visible before load — poster MUST match the video's first frame (no visual jump at start); **mobile 9:16** plays `hero-9x16-loop.mp4` object-covered (720×1280, 8s, dip-to-black fades mask the loop restart); no double-image flash between the `<picture>` still and the video start; the video scales with the art under the GSAP scrub (inside `#hero-art`); Network shows the preloaded AVIF ladder as LCP and the mp4 loading after (`preload="none"`); **no audio track** in either mp4 (ffprobe `-select_streams a` = empty); **reduced-motion**: video `display: none`, still `<picture>` shows, zero autoplay attempt; LCP still ≤ 2.5s on throttled 4G (video never blocks LCP).
...
- [ ] Countdown digit change animates transform/opacity only (DevTools inspect during tick — `key={`${cell.key}-${values[cell.key]}`}` remount replays `digit-tick`); no width change → no CLS; digits + separators render in **Geist Mono** (PR3 + PR4 D3); at 390px the countdown row fits INSIDE the hud-panel (no horizontal overflow — mobile fix: vw-driven clamps `clamp(1.6rem, 8.5vw, 3.2rem)` / `clamp(1.4rem, 6.5vw, 2.6rem)` + `.count-cell { min-width: 0 }` inside `@media (max-width: 767px)`; the base clamp 2.6rem min + min-width 4.75rem forced a 419px row inside a 342px panel).
